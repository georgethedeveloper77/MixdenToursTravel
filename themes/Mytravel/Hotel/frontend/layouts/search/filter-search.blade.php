<div class="bravo_filter navbar-expand-lg navbar-expand-lg-collapse-block">
    <button class="btn d-lg-none mb-5 p-0 collapsed" type="button" data-toggle="collapse" data-target="#sidebar"
                ...
                <div id="bravo-filter-price" class="accordion shadow-none bravo-filter-price border-bottom">
                    ...
                    <div class="border-0">
                        <div class="card-collapse">
                            <h3 class="mb-0">
                                <button type="button" class="btn btn-link btn-block card-btn py-2  text-lh-3 collapsed"
                                        data-toggle="collapse" data-target="#context-filter-price" aria-expanded="false"
                                        aria-controls="context-filter-price">
                                    <span class="row align-items-center">
                                        <span class="col-9">
                                            <span
                                                class="d-block font-size-lg-15 font-size-17 font-weight-bold text-dark">{{ __("Price Range") }} ({{$currency['symbol'] ?? ''}})</span>
                                        </span>
                            ...
                            <h3 class="mb-0">
                                <button type="button" class="btn btn-link btn-block card-btn py-2  text-lh-3 collapsed"
                                    ...
                                    <span class="row align-items-center">
                                        <span class="col-9">
                                            <span
                                                class="d-block font-size-lg-15 font-size-17 font-weight-bold text-dark text-lh-1dot4">{{ __("Hotel Star") }}</span>
                                        </span>
                                        <span class="col-3 text-right">
                                            <span class="card-btn-arrow">
                                                <span class="fa fa-chevron-down small"></span>
                            ...
                            <h3 class="mb-0">
                                <button type="button" class="btn btn-link btn-block card-btn py-2  text-lh-3 collapsed"
                                    ...
                                    <span class="row align-items-center">
                                        <span class="col-9">
                                            <span
                                                class="d-block font-size-lg-15 font-size-17 font-weight-bold text-dark text-lh-1dot4">{{ __("Review Score") }}</span>
                                        </span>
                                        <span class="col-3 text-right">
                                            <span class="card-btn-arrow">
                                                <span class="fa fa-chevron-down small"></span>
                        ...
                        <div id="review_score" class="collapse show">
                            <div class="card-body pt-0 mt-1 ">
                                @for ($number = 5 ;$number >= 2 ; $number--)
                                    <div class="form-group font-size-14 text-lh-md text-secondary mb-3">
                                        <div class="custom-control custom-checkbox">
                                            <input class="custom-control-input" id="review_score{{$number}}"
                                                   name="review_score[]" type="checkbox" value="{{$number}}"
                                                   @if(  in_array($number , request()->query('review_score',[])) )  checked @endif>
                                            <label class="custom-control-label text-lh-inherit text-color-1"
                                                   for="review_score{{$number}}">
                                                <div
                                                    class="d-inline-flex align-items-center font-size-13 text-lh-1 text-primary">
                                                    <div class="green-lighter ml-1 letter-spacing-2">
                    ...
                    @if(empty($item['hide_in_filter_search']))
                        ...
                        <div id="attr_{{$item->id}}" class="accordion rounded-0 shadow-none">
                            <div class="border-0">
                                <div class="card-collapse" id="cityCategoryHeadingOne">
                                    <h3 class="mb-0">
                                        <button type="button"
                                                ...
                                                data-toggle="collapse" data-target="#attr_more_{{$item->id}}"
                                                aria-expanded="false" aria-controls="attr_more_{{$item->id}}">
                                            <span class="row align-items-center">
                                                <span class="col-9">
                                                    <span
                                                        class="font-weight-bold font-size-17 text-dark mb-3">{{$translate->name}}</span>
                                                </span>
                                                <span class="col-3 text-right">
                                                    <span class="card-btn-arrow">
                                                        <span class="fa fa-chevron-down small"></span>
                                ...
                                <div id="attr_more_{{$item->id}}" class="collapse show"
                                     aria-labelledby="cityCategoryHeadingOne" data-parent="#attr_{{$item->id}}">
                                    <div class="card-body pt-0 mt-1  pb-4">
                                        @foreach($item->terms as $key => $term)
                                            @if($key <= 2 )
                                                ...
                                                <div
                                                    ...
                                                    <div class="custom-control custom-checkbox">
                                                        <input id="term_{{$term->id}}" class="custom-control-input"
                                                               @if(in_array($term->id,$selected)) checked
                                                               @endif type="checkbox" name="terms[]"
                                                               value="{{$term->id}}">
                                                        <label class="custom-control-label"
                                                               for="term_{{$term->id}}">{!! $translate->name !!}</label>
                                                    </div>
                                        ...
                                        <a class="link link-collapse small font-size-1 mt-2" data-toggle="collapse"
                                           href="#more_term_{{$term->id}}" role="button" aria-expanded="false"
                                           aria-controls="more_term_{{$term->id}}">
                                            <span
                                                class="link-collapse__default font-size-14">{{ __("Show all") }}</span>
                                            <span
                                                class="link-collapse__active font-size-14">{{ __("Show less") }}</span>
                                        </a>
                                    </div>
                                </div>
                            </div>
