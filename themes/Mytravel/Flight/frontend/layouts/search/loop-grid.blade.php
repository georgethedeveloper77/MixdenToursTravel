<div class="card w-100 shadow-hover-3 mb-5">
    ...
    <div class="mb-3">
        ...
        <div class="text-center font-size-14 text-primary mb-3">
            <!-- On Target Modal -->
            <a class="font-size-14 text-gray-1 d-none" href="#ontargetModal{{$row->id}}"
               data-modal-target="#ontargetModal{{$row->id}}" data-modal-type="ontarget" data-modal-effect="fadein"
               data-speed="500">{{__('Flight Details')}}</a>
        ...
        <div class="d-flex justify-content-center pl-3 pr-3">
            @if($row->can_book)
                <a @click="openModalBook('{{$row->id}}')" href="" onclick="event.preventDefault()"
                   class="btn btn-blue-1 font-size-14 width-260 text-lh-lg transition-3d-hover py-1">{{__("Choose")}}</a>
            @else
                <a href="#"
                   class="btn btn-warning font-size-14 width-260 text-lh-lg transition-3d-hover py-1 btn-disabled">{{__("Full Book")}}</a>
            @endif
        </div>
    </div>
</div>
