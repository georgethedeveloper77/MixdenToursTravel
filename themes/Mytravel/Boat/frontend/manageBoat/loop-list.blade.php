    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="control-action">
                ...
                @if(!empty($recovery))
                    <a href="{{ route("boat.vendor.restore",[$row->id]) }}" class="btn btn-recovery btn-primary"
                       data-confirm="{{__('"Do you want to recovery?"')}}">{{__("Recovery")}}</a>
                    @if(Auth::user()->hasPermissionTo('boat_delete'))
                        <a href="{{ route("boat.vendor.delete",['id'=>$row->id,'permanently_delete'=>1]) }}"
                ...
                @else
                    ...
                    @endif
                    @if(Auth::user()->hasPermissionTo('boat_delete'))
                        <a href="{{ route("boat.vendor.delete",[$row->id]) }}" class="btn btn-danger"
                           data-confirm="{{__('"Do you want to delete?"')}}">{{__("Del")}}</a>
                    @endif
                    @if($row->status == 'publish')
                        <a href="{{ route("boat.vendor.bulk_edit",[$row->id,'action' => "make-hide"]) }}"
                           class="btn btn-secondary">{{__("Make hide")}}</a>
                    @endif
                    @if($row->status == 'draft')
                        <a href="{{ route("boat.vendor.bulk_edit",[$row->id,'action' => "make-publish"]) }}"
