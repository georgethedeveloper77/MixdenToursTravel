<template>
    <div class="bravo-template-list-item">
        <div class="list-item-wraps">
            <draggable :options="{handle:'.handle'}" v-model="value">
                <div class="list-item" v-for="(item,k) in value">
                    <div class="list-item-header handle">
                        ...
                        <span>
                            <span @click="item._active = item._active == false ? true : false ">
                                <i class="icon ion-ios-arrow-dropdown" v-show="item._active"></i>
                                <i class="icon ion-ios-arrow-dropright" v-show="!item._active"></i>
                            </span>
                    ...
                    <div class="list-item-settings" v-show="item._active">
                        <vue-form-generator :model="item" :schema="{fields:schema.settings}"></vue-form-generator>
                    </div>
                </div>
            </draggable>
        </div>
        <span @click="addNew" class="btn btn-primary btn-sm">{{template_i18n.add_new}}</span>
    </div>
</template>
<script>
